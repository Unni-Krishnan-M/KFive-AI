...
export default function KnowledgePage() {
  ...
  // Reset the whole view on navigation, including transitions back to an earlier scope.
  return <KnowledgeScopePage key={location.key} />;
}

function KnowledgeScopePage() {
  const location = useLocation();
  ...
  const historyScope = knowledgeHistoryScope(location.search, requested);
  const orphaned = historyScope === 'orphaned';
  const projectValid = historyScope !== 'invalid' && (!requested || Boolean(context));
  const [status, setStatus] = useState<KnowledgeStatus>();
  const projectStatus = effectiveKnowledgeProjectStatus(context?.status, projectId, status);
  const mutationsAllowed = canMutateKnowledge(projectStatus, projectValid);
  ...
  const refresh = useCallback(async () => {
    ...
    const [statusResult, sourcesResult] = await Promise.allSettled([
      orphaned ? Promise.resolve(undefined) : knowledgeApi.getStatus(projectId),
      knowledgeApi.getSources(projectId, orphaned ? 'orphaned' : undefined),
    ]);
    if (requestId !== refreshRequestRef.current) return;
    if (statusResult.status === 'fulfilled') {
      setStatus(statusResult.value ? normalizeKnowledgeStatus(statusResult.value.data) : undefined);
      setStatusError(undefined);
    } else {
      setStatus(undefined);
    ...
    setLoading(false);
  }, [projectId, projectValid, orphaned]);
  ...
  const queryAllowed = !orphaned && projectValid && status?.canQuery === true && readySources.length > 0;
  const ingestAllowed = !orphaned && mutationsAllowed && status?.canIngest === true;
  ...
  const ingest = async () => {
    ...
    const current = () => isKnowledgeRequestCurrent(generationRef.current, generation);
    setUploading(true);
    setFileError(undefined);
    try {
      let content: string;
  ...
  const deleteSource = async (source: KnowledgeSource) => {
    if (!canDeleteKnowledgeSource(source.status, mutationsAllowed) || !window.confirm(`Delete knowledge source "${source.name}"?`)) return;
    const generation = generationRef.current;
    const current = () => isKnowledgeRequestCurrent(generationRef.current, generation);
    ...
    try {
      ...
      if (current()) setDeletingId(undefined);
    }
  };
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6 md:p-8">
    ...
    {historyScope === 'invalid' ? <div role="alert" className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">Invalid knowledge scope. Do not combine a recovery scope with a project or repeat scope parameters. No knowledge request was made.</div> : null}
    {!requested && historyScope !== 'invalid' ? <button onClick={() => navigate(orphaned ? '/app/knowledge' : '/app/knowledge?scope=orphaned', { state: null })} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-gray-300">{orphaned ? 'Show workspace knowledge' : 'Sources from deleted projects'}</button> : null}
    {orphaned ? <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">Recovery view: these sources belong to your deleted projects. You may delete completed or failed sources and their stored vectors. Uploading and querying are disabled here; workspace knowledge remains separate.</div> : null}
    ...
    {projectStatus === 'archived' ? <div role="alert" className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">{PROJECT_ARCHIVED_MESSAGE} Existing sources and queries remain available when the backend reports they are ready.</div> : null}
    {context ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-primary/30 bg-primary/10 px-4 py-3"><div className="flex items-center gap-3"><FolderKanban className="h-5 w-5 text-primary" /><div><p className="text-xs uppercase tracking-wide text-gray-500">Project knowledge</p><p className="font-medium text-white">{context.projectName} <span className="text-xs capitalize text-gray-500">({projectStatus})</span></p></div></div><button onClick={() => navigate('/app/knowledge', { replace: true, state: null })} className="inline-flex items-center gap-1 rounded-lg border border-white/10 px-3 py-1.5 text-xs text-gray-300"><X className="h-3.5 w-3.5" />Show workspace knowledge</button></div> : null}

    {!orphaned ? <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.8fr)]">
      <section className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => fileInputRef.current?.click()} disabled={!ingestAllowed || uploading} className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-gray-200 disabled:cursor-not-allowed disabled:opacity-40"><Upload className="h-4 w-4" />Choose source</button>{selectedFile ? <button onClick={() => void ingest()} disabled={!ingestAllowed || uploading} className="inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}Index {selectedFile.name}</button> : null}</div>
      ...
      <section className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
        ...
        {orphaned ? <p className="text-sm text-amber-300">Deleted-project sources cannot be queried.</p> : !status?.canQuery ? <p className="text-sm text-amber-300">Querying is unavailable. Check the dependency messages above.</p> : readySources.length === 0 ? <p className="text-sm text-amber-300">No ready knowledge source is available yet.</p> : null}
