...
import { Archive, ArchiveRestore, BookMarked, BookOpen, Bot, Clock3, Code2, Database, FileText, FolderKanban, Gauge, GitBranch, Loader2, MessageSquare, Network, Pencil, Plus, RefreshCw, Trash2, X } from 'lucide-react';
...
import { reconcileSelectedProject } from '@/services/projectRoom';
...
export default function ProjectsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelected((current) => reconcileSelectedProject(current, nextProjects));
    } catch (loadError) {
  ...
  const openProjectTool = (project: Project, path: '/app/chat' | '/app/code' | '/app/documents' | '/app/agents' | '/app/knowledge' | '/app/repositories' | '/app/workflows' | '/app/datasets' | '/app/benchmarks' | '/app/notebooks') => {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6 md:p-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="flex items-center gap-3 text-3xl font-bold text-white"><FolderKanban className="text-primary" />Projects</h1><p className="mt-2 text-gray-400">Persistent, owner-scoped rooms for KFive work.</p></div><div className="flex gap-2"><button type="button" aria-label="Refresh projects" onClick={() => void load()} className="rounded-xl border border-white/10 bg-white/5 p-2.5 text-gray-300"><RefreshCw className={`h-5 w-5 ${loading ? 'animate-spin' : ''}`} /></button><button type="button" onClick={openCreate} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 font-medium text-white"><Plus className="h-4 w-4" />New project</button></div></div>
